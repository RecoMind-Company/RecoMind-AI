"""
Precedent Engine Agent - Analyzer
"""
import re
...
class PrecedentAnalyzer:
    """Analyzes validated precedent cases: pattern extraction, confidence scoring, output building."""
    ...
    def build_output(self, cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        total = len(cases)
    ...
    def empty_result() -> Dict[str, Any]:
        return {
            ...
            "what_worked": [],
            "what_failed": [],
            "key_insights": {"success_factors": [], "failure_factors": []},
            "cases": [],
        }
    ...
    def _extract_patterns(self, cases: List[Dict[str, Any]]) -> Dict[str, List[str]]:
        ...
        def top_patterns(case_list: List[dict], top_n: int = 5) -> List[str]:
            if not case_list:
                return []
            all_bigrams = []
            for c in case_list:
                all_bigrams.extend(self._extract_bigrams(self._clean_text(c)))
            if not all_bigrams:
                all_words = []
                for c in case_list:
                    words = re.findall(r"\b[a-zA-Z]+\b", self._clean_text(c).lower())
                    all_words.extend([w for w in words if w not in self.STOPWORDS and len(w) > 5])
                return [w for w, _ in Counter(all_words).most_common(top_n)]
            return [p for p, _ in Counter(all_bigrams).most_common(top_n)]

        return {"worked": top_patterns(success_cases), "failed": top_patterns(failure_cases)}

    @staticmethod
    def _calculate_confidence(cases: List[Dict[str, Any]]) -> float:
        if not cases:
            return 0
