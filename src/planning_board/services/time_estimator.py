...
class TimeEstimatorService:
    ...
    def estimate_task_duration(
        ...
        """
        Estimate the duration of a single task
    ...
    def estimate_tasks_duration(
        ...
        """
        Estimate the duration of a set of tasks

        Args:
            tasks: List of tasks
            complexity_map: Complexity map per task (task_id -> complexity)
    ...
    def calculate_total_duration(self, tasks: List[Task]) -> int:
        ...
        Note: This is a simplified version - in reality we need to compute the Critical Path
        """
        if not tasks:
            return 0
    ...
    def _detect_complexity_from_description(self, description: str) -> str:
        """
        Infer complexity level from the description
        """
        desc_lower = description.lower()

        high_indicators = ["comprehensive", "complex", "large", "multiple", "extensive"]
        low_indicators = ["simple", "quick", "small", "basic", "minor"]
